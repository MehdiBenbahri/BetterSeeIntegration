...
{
    chrome.storage.sync.set({'doui': 0}, function() {
	});
	chrome.storage.sync.set({'dnon': 1}, function() {
	});
	chrome.tts.speak('Non');
}
...
{
    chrome.storage.sync.set({'radio-0': 1}, function() {
	});
	chrome.storage.sync.set({'radio-1': 0}, function() {
	});
		chrome.storage.sync.set({'radio-2': 0}, function() {
	});
		chrome.storage.sync.set({'radio-3': 0}, function() {
	});
		chrome.storage.sync.set({'radio-4': 0}, function() {
	});
		chrome.storage.sync.set({'radio-5': 0}, function() {
	});
		chrome.storage.sync.set({'radio-6': 0}, function() {
	});
		chrome.storage.sync.set({'radio-7': 0}, function() {
	});
		chrome.storage.sync.set({'radio-8': 0}, function() {
	});
}

else if( (document.getElementById('radio-1') !== null) && document.getElementById('radio-1').checked === true)
{
    chrome.storage.sync.set({'radio-0': 0}, function() {
	});
	chrome.storage.sync.set({'radio-1': 1}, function() {
	});
		chrome.storage.sync.set({'radio-2': 0}, function() {
	});
		chrome.storage.sync.set({'radio-3': 0}, function() {
...
else if( (document.getElementById('radio-2') !== null) && document.getElementById('radio-2').checked === true)
{
	...
	chrome.storage.sync.set({'radio-1': 0}, function() {
		...
		chrome.storage.sync.set({'radio-2': 2}, function() {
	});
		chrome.storage.sync.set({'radio-3': 0}, function() {
	});
		chrome.storage.sync.set({'radio-4': 0}, function() {
...
else if( (document.getElementById('radio-3') !== null)&& document.getElementById('radio-3').checked === true)
{
	...
	chrome.storage.sync.set({'radio-1': 0}, function() {
		...
		chrome.storage.sync.set({'radio-3': 1}, function() {
	});
		chrome.storage.sync.set({'radio-4': 0}, function() {
	});
		chrome.storage.sync.set({'radio-5': 0}, function() {
...
else if( (document.getElementById('radio-4') !== null) && document.getElementById('radio-4').checked === true)
{
	...
	chrome.storage.sync.set({'radio-1': 0}, function() {
		...
		chrome.storage.sync.set({'radio-4': 1}, function() {
	});
		chrome.storage.sync.set({'radio-5': 0}, function() {
	});
		chrome.storage.sync.set({'radio-6': 0}, function() {
...
else if( (document.getElementById('radio-5') !== null) && document.getElementById('radio-5').checked === true)
{
	...
	chrome.storage.sync.set({'radio-1': 0}, function() {
		...
		chrome.storage.sync.set({'radio-5': 5}, function() {
	});
		chrome.storage.sync.set({'radio-6': 0}, function() {
	});
		chrome.storage.sync.set({'radio-7': 0}, function() {
...
else if( (document.getElementById('radio-6') !== null) && document.getElementById('radio-6').checked === true)
{
	...
	chrome.storage.sync.set({'radio-1': 0}, function() {
		...
		chrome.storage.sync.set({'radio-6': 1}, function() {
	});
		chrome.storage.sync.set({'radio-7': 0}, function() {
	});
		chrome.storage.sync.set({'radio-8': 0}, function() {
...
else if( (document.getElementById('radio-7') !== null) && document.getElementById('radio-7').checked === true)
{
	...
	chrome.storage.sync.set({'radio-1': 0}, function() {
		...
		chrome.storage.sync.set({'radio-7': 1}, function() {
	});
		chrome.storage.sync.set({'radio-8': 0}, function() {
	});
}
...
else if( (document.getElementById('radio-8') !== null) && document.getElementById('radio-8').checked === true)
{
	...
	chrome.storage.sync.set({'radio-1': 0}, function() {
		...
		chrome.storage.sync.set({'radio-8': 1}, function() {
	});
}
...
if (result.invers === undefined || result.invers == 0)
{
  ...
  chrome.storage.sync.set({'invers': 1}, function() {
        });
}
...
chrome.storage.sync.set({'invers': 0}, function() {
        });
}
        });
...
document.getElementById("play").addEventListener("click", function () {
    ...
    chrome.tabs.executeScript({code: "document.body.innerText;"}, function (selection) {
        var userLang = navigator.language || navigator.userLanguage;
        chrome.tts.speak(selection[0], {'lang': userLang, 'rate': parseInt(rate,10)});
